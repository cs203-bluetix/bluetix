...
export default function Creators() {
    ...
    const handleFileUpload = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:9090/api/nft/upload/'+eventNameUpload, formData, {
                headers: {
    ...
    const fetchFileList = async () => {
        try {
            if (eventNameQuery) {
                const response = await axios.get('http://localhost:9090/api/nft/events/' + eventNameQuery);
